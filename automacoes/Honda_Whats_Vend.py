...
caminho_saida = r"\\192.168.200.81\\PlanejamentoPorto\\1.Carteiras\\5. Honda\\Vend"
...
SELECT 
    ...
    CASE
        WHEN DATEDIFF(CURDATE(), a.PRIM_VENC_ORIGINAL) BETWEEN 361 AND 720 THEN '1. 361 a 720 Dias'
        WHEN DATEDIFF(CURDATE(), a.PRIM_VENC_ORIGINAL) BETWEEN 721 AND 1000 THEN '2. 721 A 1000'
        WHEN DATEDIFF(CURDATE(), a.PRIM_VENC_ORIGINAL) >= 1800 THEN 'OVER 1800'
        ELSE 'VERIFICAR'
...
WHERE
    a.ID_CARTEIRA = 090
    AND a.STATUS IN (0)
    AND DATEDIFF(CURDATE(), a.PRIM_VENC_ORIGINAL) BETWEEN 361 AND 1800;
"""
...
arquivo_excel = os.path.join(caminho_saida, "honda.xlsx")
