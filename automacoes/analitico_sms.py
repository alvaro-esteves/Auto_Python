...
output_path = r'\\192.168.200.81\PlanejamentoPorto\1.Álvaro Esteves\Whats_Hoje.csv'  # <--- COLOQUE SEU CAMINHO DE SAÍDA REAL AQUI!
...
SELECT
  ...
  CONVERT(AES_DECRYPT(
    ...
    FROM_BASE64('Y7uIKQIPAASNT4ddJNqHRiyFBFqHuOsSaADXZPd59D8='),
    FROM_BASE64('pT8Ik00gAHCayRXxscLBAw==')
  ) USING UTF8) AS Mensagem_enviada,
  LENGTH(m.text) AS tamanho_mensagem,
  l.user,
  DATE_FORMAT(IF(l.startDate IS NULL, m.sentDate, l.startDate), '%d/%m/%Y') AS Data_Inicio,
  DATE_FORMAT(l.startDate, '%T') AS Hora_inicio,
  st.name AS status,
  s.price AS Custo,
  m.idMessage, 
  m.sentDate AS Data_Envio,
  l.user AS Usuario, 
  m.idStatusMessage
FROM message m
...
WHERE DATE(l.startDate) = CURDATE()
